...
class Menu extends Controller
{
    ...
    {
        $this->title = '后台菜单管理';
        $list = Db::name($this->table)->where(['status' => '1'])->order('sort asc,id asc')->field('id,pid,title,node,url')->select();
        $menus = $this->service->buildMenuData(arr2tree($list), NodeService::get(), !!session('user'));
